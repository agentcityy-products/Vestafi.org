...
type BuyFlowProgressProps = {
  /** 0 = Browse … 5 = Done */
  currentStep: number;
  className?: string;
};

export function BuyFlowProgress({ currentStep, className }: BuyFlowProgressProps) {
  const clamped = Math.max(0, Math.min(5, currentStep));
            ...
            >
              <div
                className={cn(
                  ...
                  isDone &&
                    'border-[#1EB45A] bg-[#1EB45A] text-white',
                  isActive &&
                    !isDone &&
              ...
              <span
                className={cn(
                  'max-w-[52px] text-center text-[10px] leading-snug text-muted-foreground',
