...
import { createMembershipActivationSchema } from '@/schema/membership';
import { createMembershipActivation } from '@/actions/membership';
...
type FormData = z.infer<typeof createMembershipActivationSchema>;
...
export function MembershipActivationDialog() {
  ...
  const { data: membershipStatus } = useUserMembershipStatus();
  const { isExpired } = useMembershipAccess();
  const annualFee =
    membershipStatus?.annualFee || membershipConfig.defaultAnnualFee;
  const isRenewal = isExpired;

  const form = useForm<FormData>({
    resolver: zodResolver(createMembershipActivationSchema),
    defaultValues: {
      proof_images: [],
    },
  });

  const { execute, isExecuting } = useAction(createMembershipActivation, {
    onSuccess: () => {
      toast.success(
        isRenewal
          ? 'Membership renewal request submitted successfully'
          : 'Membership activation request submitted successfully',
      );
      setOpen(false);
    ...
    onError: (error) => {
      toast.error(
        error.error.serverError ||
          'Failed to submit membership activation request',
      );
    },
  });
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      const uploadPromises = files.map(async (file) => {
        ...
        const fileName = `${loggedInUser.id}/membership-activations/${Date.now()}_${file.name}`;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-h-[90vh] overflow-y-auto sm:max-w-2xl'>
        <DialogHeader>
          <DialogTitle>
            {isRenewal
              ? 'Renew Your Vestafi Membership'
              : 'Activate Your Vestafi Membership'}
          </DialogTitle>
          <DialogDescription className='space-y-2'>
            <p>
              {isRenewal
                ? 'Complete your membership renewal by submitting payment proof for the annual membership fee.'
                : 'Complete your membership activation by submitting payment proof for the annual membership fee.'}
            </p>
            <div className='flex items-center gap-2 text-sm font-medium'>
              <span className='text-muted-foreground'>
                Annual membership fee:
              </span>
              <span className='text-primary'>{formatCurrency(annualFee)}</span>
            </div>
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
            ...
            <div className='space-y-4'>
              <Alert>
                ...
                <AlertDescription>
                  Please transfer <strong>{formatCurrency(annualFee)}</strong>{' '}
                  to the following {appConfig.title} payment details, then
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Upload screenshot of your payment transaction. At least one
