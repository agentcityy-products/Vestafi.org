...
import { MonthlyRentRow } from '@/types/dao';
...
export function MonthSelector({
  ...
  const getRentForMonth = (year: number, month: number) => {
    const dateString = `${year}-${(month + 1).toString().padStart(2, '0')}-01`;
    return rents.find((rent) => rent.month === dateString);
  };

  const hasRentForMonth = (year: number, month: number) => {
    return !!getRentForMonth(year, month);
  };

  const isDateDisabled = (year: number, month: number) => {
    const dateString = `${year}-${(month + 1).toString().padStart(2, '0')}`;

    // Disable if rent already exists for this month
    if (hasRentForMonth(year, month)) {
      return true;
    }
        ...
        <div className='p-4'>
          ...
          <div className='grid grid-cols-3 gap-2'>
            {months.map((month, index) => {
              ...
              const rentData = getRentForMonth(viewYear, index);
                ...
                >
                  ...
                  {rentData && (
                    <span className='text-xs text-muted-foreground'>
                      {formatCurrency(rentData.total_rent_collected)}
                    </span>
                  )}
                </Button>
              );
